...
The vendor driver is responsible for initializing a property map containing metadata about the slide.  A property has a name (a string) and a value (another string).  Property names defined by your driver should be prefixed by your driver’s name followed by a dot.  You should create properties for any and all metadata supported by your slide format.
...
- label := the paper label (or barcode) glued to the slide
- macro := a photograph of the entire slide, possibly extending beyond the edges of the glass
- thumbnail := a low-resolution version of the primary image
